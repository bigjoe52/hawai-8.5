...
CREATE TABLE IF NOT EXISTS side_bets (
  ...
  status         TEXT NOT NULL DEFAULT 'open'
                 CHECK (status IN ('open','matched','settled','void')),
  ...
  CONSTRAINT settled_needs_winner CHECK (
    (status = 'settled' AND winner IS NOT NULL) OR status <> 'settled'
  ),
  CONSTRAINT matched_needs_taker CHECK (
    (status IN ('matched','settled') AND taker_id IS NOT NULL)
    OR status IN ('open','void')
  )
);
